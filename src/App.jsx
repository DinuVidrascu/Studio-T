...
export default function App() {
      ...
      if (user) {
        try {
          ...
          if (!userSnap.exists()) {
            ...
            if (isConfigured) {
              await setDoc(doc(db, "notifications", `new-user-${user.uid}`), {
                id: `new-user-${user.uid}`,
                type: 'new_user',
                userId: user.uid,
                message: `👤 Utilizator nou: ${user.displayName || user.email} a solicitat acces în Studio.`,
                time: Date.now(),
                read: false
              });
            }
  ...
  useEffect(() => {
    ...
    const unsubUsers = onSnapshot(collection(db, "users"), async (snapshot) => {
      if (snapshot.empty) {
        ...
        // Auto-create notifications for unapproved users missing one
        const unapproved = usrs.filter(u => u.approved !== true && u.role !== 'admin');
        for (const u of unapproved) {
          const notifId = `new-user-${u.uid}`;
          try {
            ...
            if (!notifSnap.exists()) {
              await setDoc(notifRef, {
                ...
                type: 'new_user',
                userId: u.uid,
                message: `👤 Utilizator nou: ${u.displayName || u.email} a solicitat acces în Studio.`,
    ...
    const unsubNotifications = onSnapshot(collection(db, "notifications"), (snapshot) => {
      if (snapshot.empty) {
        ...
        const notifs = [];
        snapshot.forEach((d) => notifs.push(d.data()));
  ...
  const handleMarkAsRead = async (id) => {
    const isLocal = localNotifications.some(n => n.id === id);
    if (isLocal) {
  ...
  const handleMarkAllRead = async () => {
    ...
    for (const n of dbNotifications.filter(n => !n.read)) {
  ...
  const handleApproveUser = async (userId, notifId) => {
    try {
      await updateDoc(doc(db, "users", userId), { approved: true });
      if (notifId) await deleteDoc(doc(db, "notifications", notifId));
      setToast({ message: 'Utilizatorul a primit acces în Studio!', type: 'success' });
    } catch (e) {
      setToast({ message: 'Eroare la aprobare.', type: 'error' });
    }
  };
  ...
  const visibleEvents = userRole === 'admin'
    ? events.filter(e => e.status !== 'refused')
    : events.filter(e => {
        ...
        return isVisible && (isCreator || (userInEventTeam && (e.status === 'confirmed' || e.status === 'pending')));
      });
  ...
  const visibleNotifications = useMemo(() => {
    ...
    return notifications.filter(n => {
      if (n.type === 'new_user') return false; // only admins see new_user notifs
      if (n.team) return n.team.includes(myMemberId);
      if (n.projectId) return projects.some(p => p.id === n.projectId && p.team && p.team.includes(myMemberId));
      if (n.eventId) return events.some(e => e.id === n.eventId && e.team && e.team.includes(myMemberId));
      return false;
    });
  }, [userRole, notifications, projects, events, myMemberId]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', minHeight: '100vh', width: '100%', background: C.paper, position: 'relative', overflow: 'hidden' }}>
      ...
      <NotificationsBell 
        ...
        onApproveUser={userRole === 'admin' ? handleApproveUser : undefined}
        isMobile={isMobile} 
      />
